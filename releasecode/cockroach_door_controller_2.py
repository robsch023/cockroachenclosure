...
# ── Vision / detection ────────────────────────────────────────────────────────
MODEL_PATH        = "/home/rosch023/enclosure/model_float.tflite"  # update filename if different
# NOTE: 0.45 was correct for the old INT8 model's decoded [0,1] probability
# scale. The float model's output tensor is raw/undecoded (no NMS or sigmoid
# applied), so its scores live on a much smaller scale — 0.11 was the
...
def run_inference(frame_bgr: np.ndarray):
    ...
    if boxes_tensor.dtype == np.uint8:
        ...
        # Column convention: 0 = score, 1 = class id.
        ...
        scores  = sc_raw[:, 0]                       # score
        classes = sc_raw[:, 1]                       # class id
    else:
        # Float32 model — raw/undecoded output (no NMS or sigmoid applied).
        # Column convention here is 0 = background score, 1 = cockroach score
        # (matches the confusion-matrix evaluation script, empirically
        # validated against labelled data — threshold 0.11).
        boxes_raw = boxes_tensor[0].astype(np.float32)
        sc_raw    = sc_tensor[0].astype(np.float32)
        scores    = sc_raw[:, 1]                      # cockroach score
        classes   = np.ones(sc_raw.shape[0])           # always class 1 (cockroach)

    h, w = frame_bgr.shape[:2]
    detections = []
    is_float_model = boxes_tensor.dtype != np.uint8
    for i, score in enumerate(scores):
        if score < CONFIDENCE_THRESH:
            continue
        cls_id = TARGET_LABEL if is_float_model else None
        if cls_id is None:
        ...
        if is_float_model:
            # NOTE: these are raw, undecoded anchor-relative regression deltas,
            # NOT normalised [0,1] box coordinates — they are NOT real pixel
            # positions yet. Detection (score-based trigger) is reliable;
            # the drawn bounding box position/size is not, until the model
            # is re-exported with the postprocessing/decode op included.
            box_px = [0, 0, w, h]   # placeholder — draws full-frame box
        else:
            box_px = [int(xmin * w), int(ymin * h),
